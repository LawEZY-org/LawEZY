...
java $JVM_FLAGS -jar lawezy-identity-service.jar --server.port=8081 > identity.log 2>&1 &

# 2. Start Chat Service (port 8082)
echo "Starting lawezy-chat-service..."
java $JVM_FLAGS -jar lawezy-chat-service.jar --server.port=8082 > chat.log 2>&1 &

# 3. Start Finance Service (port 8083)
echo "Starting lawezy-finance-service..."
java $JVM_FLAGS -jar lawezy-finance-service.jar --server.port=8083 > finance.log 2>&1 &

# 4. Start Notification Service (port 8084)
echo "Starting lawezy-notification-service..."
java $JVM_FLAGS -jar lawezy-notification-service.jar --server.port=8084 > notification.log 2>&1 &
